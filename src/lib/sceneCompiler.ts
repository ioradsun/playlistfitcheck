...
export class LyricDancePlayer {
  ...
  private async ensureTimelineReady(): Promise<void> {
    ...
    if (!globalBakePromise) {
      ...
      globalBakePromise = (async () => {
        // Ensure we have real viewport dimensions before compiling.
        // ResizeObserver fires async, so this.width may still be 0.
  ...
  private _updateViewportScale(): void {
    ...
    if (isPortrait) {
      ...
      // The viewportMinFont clamp (26px) ensures readability.
      fontScale = sx;
    } else {
