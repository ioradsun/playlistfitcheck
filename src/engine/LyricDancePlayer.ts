...
export class LyricDancePlayer {
  ...
  private _draw(tSec: number, precomputedFrame: ScaledKeyframe | null): void {
    ...
    const drawChunkText = (chunk: ScaledKeyframe['chunks'][number], glowPass: boolean) => {
      ...
      // ═══ INLINE ELEMENTAL TEXTURE: applied in the same transform as the word ═══
      const _elKey = this.cleanWord((chunk.text ?? '') as string);
      const _elDir = _elKey ? this.resolvedState.wordDirectivesMap[_elKey] ?? null : null;
      if (_elDir?.elementalClass && drawAlpha > 0.1) {
        const elClass = _elDir.elementalClass as string;
        const textW = (this.chunks.get(chunk.id)?.width ?? 40) * (chunk.scaleX ?? chunk.scale ?? 1);
        const beatPulseEl = this._lastBeatState?.pulse ?? 0;
        const elTime = tSec ?? 0;
        ...
        switch (elClass) {
          case 'FROST':
          case 'ICE': {
            // ── Ice shimmer: gradient sweep across text ──
            const sweepX = (Math.sin(elTime * 1.2) * 0.5 + 0.5) * textW;
            const frostGrad = this.ctx.createLinearGradient(
          ...
          case 'RAIN': {
            ...
            // Sheen overlay
            const sheenGrad = this.ctx.createLinearGradient(0, -safeFontSize, 0, 0);
