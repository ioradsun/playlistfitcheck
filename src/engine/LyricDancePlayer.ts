...
if (primaryLineIndex >= 0) {
  ...
  for (let ri = 0; ri < activeGroups.length; ri++) {
    ...
    // Keep groups on the current line that are temporally active
...
for (let ai = 0; ai < activeGroups.length; ai++) {
  ...
  for (let wi = 0; wi < group.words.length; wi++) {
    ...
    // Hero words use wave-driven heroPresentation — don't add beat pulse on top
    if (!isHeroWord && emphasisPulse > 0) {
