import type { CinematicDirection, CinematicSection, StoryboardEntry, WordDirective } from "@/types/CinematicDirection";
 ...
 * Handles:
 *   - null / undefined → null
 *   - legacy array format (old schema) → null
 *   - legacy {chapters:[...]} format → converted to {sections:[...]}
 *   - {sections:[]} empty → null
 *   - valid {sections:[...]} → returned as-is
 */
export function normalizeCinematicDirection(
  ...
  if (
    ...
    // Map legacy Chapter[] to CinematicSection[]
  ...
  // New format — must have at least one valid section
