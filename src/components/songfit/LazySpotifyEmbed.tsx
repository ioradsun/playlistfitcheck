...
function LazySpotifyEmbedInner({ trackId, trackTitle, trackUrl, postId, albumArtUrl, cardState: _cardState }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 bg-card pointer-events-none" style={{ height: 10, opacity: 0.8 }} />
      <div className="absolute bottom-0 left-0 right-0 bg-card pointer-events-none" style={{ height: 10, opacity: 0.8 }} />
      {iframeLoaded && (
