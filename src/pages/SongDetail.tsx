...
function BattleDrillDown({ post, navigate }: { post: SongFitPost; navigate: (path: any) => void }) {
  const [hookA, setHookA] = useState<{ id: string; hook_phrase: string; hook_label: string | null; vote_count: number } | null>(null);
  ...
  useEffect(() => {
    if (!post.lyric_dance_url) { setLoading(false); return; }

    const segments = post.lyric_dance_url.replace(/^\//, "").split("/").filter(Boolean);
    if (segments.length < 3) { setLoading(false); return; }
    ...
    (async () => {
      ...
      const { data: hooks } = await supabase
        .from("shareable_hooks" as any)
        .select("id, hook_phrase, hook_label, vote_count, battle_position")
        .eq("battle_id", battleId)
        .order("battle_position", { ascending: true });

      if (hooks && hooks.length >= 2) {
        const sorted = (hooks as any[]).sort((a, b) => a.battle_position - b.battle_position);
        setHookA(sorted[0]);
        setHookB(sorted[1]);
        setTotalVotes((sorted[0].vote_count ?? 0) + (sorted[1].vote_count ?? 0));
      }

      await supabase
        .from("lyric_dance_angle_votes" as any)
        .select("hook_index, created_at")
        .eq("dance_id", battleId)
        .order("created_at", { ascending: true });
  ...
  const votePctA = totalVotes > 0 ? Math.round(((hookA?.vote_count ?? 0) / totalVotes) * 100) : 0;
  const votePctB = totalVotes > 0 ? Math.round(((hookB?.vote_count ?? 0) / totalVotes) * 100) : 0;
  const winner = totalVotes >= 3 ? (votePctA > votePctB ? "A" : votePctB > votePctA ? "B" : "tie") : null;
    ...
    <div className="min-h-screen bg-background">
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-lg font-semibold truncate">{post.track_title}</p>
            <p className="text-[10px] font-mono text-purple-400 uppercase tracking-wider">FMLY Feud</p>
          </div>
        </div>
        ...
        <div className="space-y-1.5">
          <div className="flex items-baseline gap-3">
            ...
            <span className="text-[24px] font-mono font-medium text-foreground">{totalVotes}</span>
            <span className="text-[11px] font-mono text-muted-foreground">votes</span>
          </div>
          {totalVotes >= 3 && winner && winner !== "tie" && (
            <p className="text-[12px] text-muted-foreground/60 leading-relaxed">
              The FMLY has spoken — {winner === "A" ? "Side A" : "Side B"} is winning with {winner === "A" ? votePctA : votePctB}% of the vote.
              {totalVotes < 10 ? " Still early — more votes will tell the full story." : ""}
            </p>
          )}
          {winner === "tie" && (
            <p className="text-[12px] text-muted-foreground/60 leading-relaxed">
              Dead heat — the FMLY is split. Both hooks are resonating.
            </p>
          )}
        </div>
        ...
        {hookA && hookB && (
          <div className="glass-card rounded-xl p-4 space-y-4">
            <p className="text-[9px] font-mono text-muted-foreground uppercase tracking-wider">hook battle</p>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono text-purple-400/70">
                  {hookA.hook_label ?? "Side A"} {winner === "A" ? "👑" : ""}
                </span>
                <span className="text-[11px] font-mono text-foreground/60">{hookA.vote_count} · {votePctA}%</span>
              </div>
              <p className="text-[13px] text-foreground/80 italic leading-snug">"{hookA.hook_phrase}"</p>
              <div className="h-2 rounded-full bg-muted/30 overflow-hidden">
                <div className="h-full rounded-full transition-all duration-700" style={{ width: `${votePctA}%`, background: winner === "A" ? "rgba(168,85,247,0.7)" : "rgba(168,85,247,0.35)" }} />
              </div>
            </div>
            <div className="flex items-center gap-3">
              <div className="flex-1 h-px bg-border/20" />
              <span className="text-[9px] font-mono text-muted-foreground/30">vs</span>
              <div className="flex-1 h-px bg-border/20" />
            </div>
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-mono text-purple-400/70">
                  {hookB.hook_label ?? "Side B"} {winner === "B" ? "👑" : ""}
                </span>
                <span className="text-[11px] font-mono text-foreground/60">{hookB.vote_count} · {votePctB}%</span>
              </div>
              <p className="text-[13px] text-foreground/80 italic leading-snug">"{hookB.hook_phrase}"</p>
              <div className="h-2 rounded-full bg-muted/30 overflow-hidden">
                <div className="h-full rounded-full transition-all duration-700" style={{ width: `${votePctB}%`, background: winner === "B" ? "rgba(168,85,247,0.7)" : "rgba(168,85,247,0.35)" }} />
              </div>
            </div>
            {totalVotes >= 5 && winner && winner !== "tie" && (() => {
              const margin = Math.abs(votePctA - votePctB);
              if (margin > 30) {
                return <p className="text-[11px] text-muted-foreground/50 pt-1 leading-relaxed">Clear winner — {margin}pt margin. Use the winning hook as your caption angle.</p>;
              }
              return <p className="text-[11px] text-muted-foreground/50 pt-1 leading-relaxed">Close race — only {margin}pts apart. Both hooks resonate with different listeners.</p>;
            })()}
          </div>
        )}
        ...
        {comments.length > 0 && (
          <div className="glass-card rounded-xl p-4 space-y-2">
            <p className="text-[9px] font-mono text-muted-foreground uppercase tracking-wider">battle comments · {comments.length}</p>
            {comments.slice(0, 8).map((c) => (
              <div key={c.id} className="py-1.5 border-b border-border/15 last:border-0">
                <div className="flex items-center gap-2">
                  ...
                  {c.line_index != null && (
                    <span className="text-[8px] font-mono text-purple-400/40">
                      {c.line_index === 0 ? "Side A" : "Side B"}
                    </span>
                  )}
                </div>
                <p className="text-[11px] text-foreground/70 leading-snug">{c.text}</p>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="glass-card rounded-xl p-4 space-y-2">
          ...
          <div className="flex justify-between text-[11px]">
            ...
            <span className="font-mono text-foreground/70">{post.likes_count}</span>
          </div>
          <div className="flex justify-between text-[11px]">
            <span className="text-muted-foreground">Comments</span>
            <span className="font-mono text-foreground/70">{post.comments_count}</span>
          </div>
          {post.impressions > 0 && totalVotes > 0 && (
            <p className="text-[11px] text-muted-foreground/50 pt-1 leading-relaxed">
              {Math.round((totalVotes / post.impressions) * 100)}% of viewers voted — {
                (totalVotes / post.impressions) >= 0.15
                  ? "high participation. Your hooks are engaging."
                  : "share wider to get a definitive winner."
              }
            </p>
          )}
        </div>
        ...
        {totalVotes === 0 && (
          <div className="flex flex-col items-center gap-3 py-12">
            ...
            <p className="text-[12px] text-muted-foreground text-center leading-relaxed max-w-[260px]">
              Share your battle to get votes. The FMLY will decide which hook hits hardest.
            </p>
          </div>
        )}
      </div>
...
const SongDetail = () => {
  ...
  const isBattle = !!(post?.lyric_dance_url && !post?.lyric_dance_id && !post?.spotify_track_id);
  ...
  if (isBattle) {
    return <BattleDrillDown post={post} navigate={navigate} />;
  }
